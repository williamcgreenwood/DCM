...
def rank_candidates(candidates: list[dict[str, Any]], *, top_k: int = 25, draws: int = 256, seed: str = "DCM_RANK") -> list[dict[str, Any]]:
    ...
    ranked = sorted(candidates, key=lambda p: (float(p.get("selectionScore") or -999), str(p["row"].get("projectionId"))), reverse=True)
    if not ranked:
        return ranked
    ...
    for _ in range(samples):
        ...
        for _, p in sampled[: min(top_k, len(sampled))]:

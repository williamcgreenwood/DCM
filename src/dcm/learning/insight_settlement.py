...
def build_insight_settlement(
    ...
    *,
    ...
    base: dict[str, Any] = {
        ...
        "outcomeRetrievedAt": "",
        "outcomePublishedAt": "",
        "outcomeConfidence": None,
        "outcomeEventStatus": "",
        "predictionP": _num(claim.get("predictionP")),
        "selectedP": _num(claim.get("selectedP")),
        "calibratedP": _num(claim.get("calibratedP")),
            ...
            base.update(
                {
                    ...
                    "outcomeSourceUrl": _text(outcome.get("sourceUrl"), 1024),
                    "outcomeSourceHash": _text(outcome.get("sourceHash"), 128),
                    "outcomeAuthority": _text(outcome.get("authority"), 128),
                    "outcomeRetrievedAt": _text(outcome.get("retrievedAt"), 64),
                    "outcomePublishedAt": _text(outcome.get("publishedAt"), 64),
                    "outcomeConfidence": _num(outcome.get("confidence")),
                    "outcomeEventStatus": _text(outcome.get("eventStatus") or (outcome.get("event") or {}).get("status"), 64) if isinstance(outcome.get("event"), dict) else _text(outcome.get("eventStatus"), 64),

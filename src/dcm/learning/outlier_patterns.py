...
class SegmentSummary:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {"segment": dict(self.segment), "n": self.n, "wins": self.wins, "losses": self.losses, "posteriorMean": self.posterior_mean, "wilsonLower": self.wilson_lower, "state": self.state, "promotionForbidden": True}
...
def summarize_leg_segments(rows: Iterable[Mapping[str, Any]], *, by: tuple[str, ...], minimum_n: int) -> list[SegmentSummary]:
    ...
    return [SegmentSummary(key, n, wins, n - wins, (wins + 1) / (n + 2), _wilson_lower(wins, n), "CHALLENGER_REQUIRES_FUTURE_HOLDOUT") for key, (n, wins) in sorted(groups.items()) if n >= minimum_n]
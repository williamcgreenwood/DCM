...
test("synthetic HAR ingest accounts every row", () => {
  const ing = ingestHar(DEMO_HAR);
  assert.equal(ing.adapter, "SYNTHETIC");
  assert.ok(ing.rows.length >= 30);
});

test("full board is processed; Goblins never enter the card", () => {
  const run = runFromHar(DEMO_HAR);
...
test("empty-or-short card is legal — never pad to six", () => {
  const run = runFromHar(DEMO_HAR);
  assert.ok(run.card.length <= 6);
  assert.equal(run.card.length, Math.min(6, run.top25Qualified.length));
});

test("unsupported sports fail closed after inventory", () => {
  const run = runFromHar(DEMO_HAR);
...
test("baseball 0.5 H+R+RBI half-line policy excludes without being a theorem", () => {
  const run = runFromHar(DEMO_HAR);
...
test("probabilities simplex on modeled rows", () => {
  const run = runFromHar(DEMO_HAR);
...
test("run is deterministic", () => {
  const a = runFromHar(DEMO_HAR);
  const b = runFromHar(DEMO_HAR);
...
test("completion gates are all true on demo", () => {
  const run = runFromHar(DEMO_HAR);

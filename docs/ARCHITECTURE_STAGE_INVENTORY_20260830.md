...
| LearningLedger | PARTIAL | `dcm/learning/sidecar.py` append-only sqlite + `learning_ledger.jsonl`. Freeze writes FrozenForecast; settle appends one Settlement per modeled prop and never rewrites the frozen forecast. No full-season / multi-slate training ledger. |

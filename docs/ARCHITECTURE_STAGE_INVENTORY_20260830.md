...
| source research | PARTIAL/COMPLETE | File/Bundle/Fixture providers + `authority.py`; basketball SourceAdapters plus gridiron `FootballReferenceGameLogAdapter` / `ProFootballReferenceAdapter` (PFR/CFB HTML fixtures; live fetch opt-in `DCM_LIVE_FETCH`). Model code does not parse host pages. Remaining families (MLB/etc.) still missing adapters. |
...
| Role / availability | COMPLETE | `RoleEpochBuilder.v2-20260830` basketball path unchanged. P7 adds gridiron mode: starter/depth, QB identity, snap/route/target/carry share change-points (plus qb_id cuts). Availability remains a status string on the snapshot. |
| ParameterSnapshots | PARTIAL/COMPLETE | Basketball path unchanged. Gridiron path now calls `RoleEpochBuilder` + `GridironOpportunityModel`/`GridironEfficiencyModel`/`TeamEventModel` (QB vs skill). Opponent pass/rush defense is a required placeholder — missing → `OPPONENT_PASS_DEFENSE`/`OPPONENT_RUSH_DEFENSE` fail-closed, not PLAYABLE. MLB still sketched. |

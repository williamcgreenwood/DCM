...
| Combat, soccer, hockey, tennis, golf, esports, motorsport, etc. | RESEARCH_ONLY with executable contracts / MISSING production depth | Seventeen canonical sport families now have alias-safe identity profiles, native research requirements, source routing, subject types, event/segment/world semantics and import-validated 24-component gap ledgers. Research-shadow acquisition is executable; production modeling remains fail closed until native physics, markets, settlement and acceptance are implemented. |
